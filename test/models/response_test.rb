...
class ResponseTest < ActiveSupport::TestCase
  def setup
    @response = responses(:in_progress_response)
  end
  ...
  test "should belong to questionnaire" do
    ...
    assert_equal questionnaires(:draft_questionnaire), @response.questionnaire
  end
  ...
  test "should scope by account" do
    results = Response.for_account(accounts(:one))
    assert_includes results, @response
    assert_not_includes results, responses(:account_two_response)
  end

  test "should scope by status" do
    results = Response.by_status(:in_progress)
    assert_includes results, @response
    assert_not_includes results, responses(:completed_response)
  end
