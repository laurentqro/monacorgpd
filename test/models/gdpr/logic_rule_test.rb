...
class Gdpr::LogicRuleTest < ActiveSupport::TestCase
  def setup
    @logic_rule = gdpr_logic_rules(:show_section_rule)
  end
  ...
  test "should belong to source_question" do
    ...
    assert_equal gdpr_questions(:single_choice_question), @logic_rule.source_question
  end

  test "should optionally belong to target_section" do
    assert_respond_to @logic_rule, :target_section
    assert_equal gdpr_sections(:section_two), @logic_rule.target_section
  end
  ...
  test "should have hide action" do
    rule = gdpr_logic_rules(:hide_section_rule)
    assert rule.hide?
  end

  test "should have skip_to_section action" do
    rule = gdpr_logic_rules(:skip_to_section_rule)
    assert rule.skip_to_section?
  end

  # Scopes
  test "should scope by question" do
    results = Gdpr::LogicRule.for_question(gdpr_questions(:single_choice_question))
    assert_includes results, @logic_rule
  end

  test "should scope by section" do
    results = Gdpr::LogicRule.for_section(gdpr_sections(:section_two))
    assert_includes results, @logic_rule
  end
